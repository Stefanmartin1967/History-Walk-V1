...
// Cycle de vie destination — MODÈLE C. Une destination est TOUJOURS une entrée
// GitHub ; status:'draft' la masque des non-admins. Ce module gère les 3 écritures
// GitHub de son cycle de vie :
//   - registerDraftDestinationOnGitHub : CRÉATION (push immédiat, status:'draft',
//     geojson vide → zones → index circuits → destinations.json EN DERNIER) ;
//   - setDestinationPublished : « Rendre publique » (geojson ÉPURÉ des candidats →
...
// destinations.json EN DERNIER (rien ne pointe vers un fichier manquant) ; à la
// suppression, destinations.json EN PREMIER (plus aucun client ne pointe vers les
// fichiers avant de les retirer).
//
// SOURCE DU GEOJSON (publication) : generateMasterGeoJSONData (state.loadedFeatures
// + overlay userData = l'état AFFICHÉ) — même générateur que « Tout publier » du CC.
